...
async def _runtime_image_selection() -> tuple[str | None, dict[str, Any] | None]:
    """Resolve the current platform user's tenant-local image model selection."""
    from app.core.db import async_session_factory
    from app.core.runtime_context import platform_user_id
    from app.services.image_model_service import image_model_service
    from app.services.profile_service import profile_service

    user_id = platform_user_id()
    async with async_session_factory() as db:
        provider = await profile_service.get_user_image_provider(db, user_id=user_id)
...
async def generate_images_node(state: AgentState) -> Dict[str, Any]:
    ...
    with MetricsContext("generate_images") as tracker:
        try:
            ...
            provider, image_model_config = await _runtime_image_selection()
