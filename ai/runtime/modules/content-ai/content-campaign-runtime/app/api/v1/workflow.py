...
async def start_workflow(
    ...
    """
    get_llm_service().require_configured()
    try:
...
async def resume_workflow(
    ...
    if request.action == "approve":
        ...
        get_image_service().require_configured(provider, image_model_config)
    try:
        # 验证 thread_id 属于当前用户
        if not thread_id.startswith(str(current_user.id)):
            raise HTTPException(
...
async def get_all_threads(
    ...
    try:
        ...
        async for checkpoint in checkpointer.alist(None, limit=1000):
            ...
            if thread_id.startswith(user_prefix):
                thread_ids.add(thread_id)

        graph = await get_graph()
        for thread_id in sorted(thread_ids, reverse=True):
            config = {"configurable": {"thread_id": thread_id}}
            try:
                state_snapshot = await graph.aget_state(config)
