...
public class FileController {
    ...
    public BaseResult uploadFile(
        ...
        try {
            ...
            // 上传并处理文件
            FileInfo fileInfo = fileManageService.uploadFile(file, tenantId);
    ...
    public BaseResult<FileInfo> getFileInfo(
            @RequestHeader("X-Tenant-Id") String tenantId,
    ...
    public BaseResult<Map<String, Object>> getFileContent(
            @RequestHeader("X-Tenant-Id") String tenantId,
            @PathVariable String fileId
    ) {
    ...
    public BaseResult<String> deleteFile(
            @RequestHeader("X-Tenant-Id") String tenantId,
            @PathVariable String fileId
    ) {
    ...
    public BaseResult<Map<String, Object>> listFiles(@RequestHeader("X-Tenant-Id") String tenantId) {
        log.info("获取所有文件列表");

        try {
            var files = fileManageService.getAllFiles().entrySet().stream()
                    .filter(entry -> TenantResourceScope.ownsFile(tenantId, entry.getKey()))
    ...
    public BaseResult<Boolean> fileExists(
            @RequestHeader("X-Tenant-Id") String tenantId,
            @PathVariable String fileId
    ) {
