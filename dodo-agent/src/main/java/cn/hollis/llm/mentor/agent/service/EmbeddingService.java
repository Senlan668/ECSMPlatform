...
public class EmbeddingService {
    ...
    @Value("${platform.features.pgvector-enabled:false}")
    ...
    public void init() {
        if (!pgVectorEnabled) {
            log.info("PgVector 未启用，大文件向量检索能力已关闭");
            return;
        }
        try {
            vectorStore = pgVectorStoreFactory.createPgVectorStore("vector_file_info");
        } catch (RuntimeException exception) {
            log.warn("PgVector 初始化失败，大文件向量检索能力已关闭: {}", exception.getMessage());
        }
    }
    ...
    public void embedAndStore(List<Document> documents) {
        if (vectorStore == null) throw new IllegalStateException("PgVector 未配置或不可用");
    ...
    public List<String> ragRetrieve(String fileId, String question) {
        ...
        if (vectorStore == null) return Collections.singletonList("PgVector 未配置或不可用");
